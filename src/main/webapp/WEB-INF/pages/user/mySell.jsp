            ...
            <c:if test="${!empty goodsList}">
                <c:forEach var="item" items="${goodsList}">
                    <div class="prolist_html mat20">
                        <div class="prolist"
                             style="padding: 15px;height: 400px;width:260px;float: left;background: #fff;position: relative;margin-right: 20px;margin-top: 15px;">
                            <a href="<%=basePath%>/goods_queryGoodsById?id=${item.goods.id}"
                               target="_blank">
                                ...
                                <p class="prolist_name" style="font-size: 16px;line-height: 30px;overflow: hidden;text-overflow: ellipsis;white-space: nowrap;"><label>商品名称：</label>
                                ...
                                <div style="margin: 5px auto">
                                    ...
                                    <c:if test="${item.goods.status ==1}">
                                        <a class="set"
                                           href="<%=basePath%>goods_updateGoodsTime?id=${item.goods.id}&status=${item.goods.status}">擦亮</a></span>
                                    </c:if>
